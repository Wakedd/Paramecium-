...
doubleaspectRatio= size.width/ size.height;
returnaspectRatio;
}
...
cv::Scalar contourColor = cv::Scalar(0, 165, 255); // Orange color (BGR 
format)
cv::Scalar centroidColor = cv::Scalar(0, 0, 255); // Red color (BGR 
format)
cv::cvtColor(g_frame, g_outlinesimage, cv::COLOR_GRAY2BGR);
 // Filter contours based on size and aspect ratio, and draw them on the 
contour image
...
voidonThresholdChange(int, void*)
{
processImage();
}
// Callback function for the blur trackbar
voidonBlurSizeChange(int, void*)
{
if(g_blurSize% 2== 0)
++g_blurSize;
if(g_blurSize< 3)
...
voidonFgMaskBlurSizeChange(int, void*)
{
...
cv::createTrackbar("Threshold", "Segmented Image", &g_thresholdValue, 255, 
onThresholdChange);
